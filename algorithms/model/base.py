...
class BaseModel(object):
    ...
    def experience(self,net, data, times=config.TIMES, step=config.STEP,new_try_numbers = config.NEW_TRY_NUMBERS,
        ...
        try:
            from algorithms.network import PNNetModel
            from algorithms.network import GRNetModel
            try:
                net.apply(PNNetModel.weights_init)
            except:
        ...
        for t in range(0, times, step):
            ...
            for i in range(try_numbers):
                ...
                print('[当前{}时刻,已映射{}个虚拟网络，当前尝试映射的第{}号虚拟网络，还剩下{}个虚拟网络等待嵌入]'.format(t, success_num, id+1,valid_queue.qsize()))
            ...
            # 计算性能
            longterm_rc = utils.get_revenue_cost_ratio(longterm_revenue, longterm_cost)
            revenue = longterm_revenue / (t+1)
            rc.append('{:.3f}'.format(longterm_rc))
            revenues.append('{:.3f}'.format(revenue))
            all_time.append(t)
    ...
    # 程序入口
    def experience2(self, net, data, times=config.TIMES, step=config.STEP, new_try_numbers=config.NEW_TRY_NUMBERS,
                   max_try_numbers=config.MAX_TRY_NUMBERS, first_request_num=100, full_request=True, dropout=.2,
                   load_model_path='LVne',current_iter_time=1,numbers=0):
        ...
        VN_Link = data.get("VN_Link")
        VN_Life = data.get("VN_Life")
        vn_arrive_time = data.get("VN_Arrive_Time")
        ...
        count_time = vn_arrive_time[0]
        vn_arrive_time = list(vn_arrive_time)
        vn_arrive_time.append(0)
        ...
        for t in range(0, times, step):
            ...
            if t == count_time:
                ...
                for i in range(try_numbers):
                    ...
                    count_time = int(vn_arrive_time[id+1])

                    print('[当前{}时刻,已映射{}个虚拟网络，当前尝试映射的第{}号虚拟网络，还剩下{}个虚拟网络等待嵌入]'.format(t, success_num, id,
                                                                                      valid_queue.qsize()))
                    # print('v_node_num = ', len(v_nodes), 'v_link_num = ', len(v_links), 'v_life_time = ',life_time)
                    ...
                    if not embedding_status:
                        ...
                        print("当前映射情况，映射成功虚拟请求数量：{}，总虚拟请求数量：{}，当前请求率：{}".format(success_num,
                                                                                total_num,round(success_num/total_num,2)))
            ...
            longterm_rc = utils.get_revenue_cost_ratio(longterm_revenue, longterm_cost)
            rc.append('{:.3f}'.format(longterm_rc))
            revenus.append(longterm_revenue)
            all_time.append(t)
            if total_num == 0:
                accept_ratio.append(total_num)
            else:
                accept_ratio.append(success_num / total_num)
        ...
        df.to_csv("../../data/result/{}_{}.csv".format(load_model_path, current_iter_time))
